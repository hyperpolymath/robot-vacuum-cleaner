...
# setup.sh — Universal setup script for robot-vacuum-cleaner
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/robot-vacuum-cleaner/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== robot-vacuum-cleaner Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/robot-vacuum-cleaner.git"
        info "Then: cd robot-vacuum-cleaner && ./setup.sh"
        exit 1
    fi
